...
@php
   $totalPendingOrder = $orders->count();  
@endphp
<div class="col-xs-12">
    <div class="clearfix">
    ...
    <div class="table-header">
        All Complete Orders ({{$totalPendingOrder}})
    </div>
    <div class="">
            ...
            <tbody>
                @foreach($orders as $sl=>$order)
                    <tr>
                        ...
                        <td class="center">
                            {{ $order->order_id }} <br>
                            {{ $order->rel_to_customer ? $order->rel_to_customer->name : 'N/A' }} <br>
                            {{ $order->rel_to_customer ? $order->rel_to_customer->email : 'N/A' }}
                        </td>
                        <td class="center">{{ $order->sub_total }}</td>
                        <td class="center">{{ $order->charge }}</td>
                        <td class="center">{{ $order->total }}</td>
                        <td class="center">{{ $order->payment_method == 1 ? 'Unpaid' : 'Bkash' }}</td>
                        <td class="center">
                            @php
                                $status_labels = [
                                    1 => 'Pending',
